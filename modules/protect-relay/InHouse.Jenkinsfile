pipeline {
    ...
    post ('Email notification') {
        success {
            echo "Image building success for protect relay, sending email to $NOTIFY_EMAILS"
            notifyBuild('SUCCESS')
        }
        failure {
            echo "Image building failed for protect relay, sending email to $NOTIFY_EMAILS"
            notifyBuild('FAILED')
        }
    }
}
