pipeline {
    ...
    environment {
      git_url = "gitlab-address/notification-service"
      branch = "$BRANCH_NAME"
      version = "$VERSION"
      registry = "$REGISTRY"
    ...
    stages {
        ...
        stage('clone repository') {
            steps {
              dir('modules/notification-service-tmp/workdir') {
        ...
        stage('build image') {
            steps {
              echo 'Building image..'
              sh "echo $PWD && ls -alF"
              dir('modules/notification-service-tmp') {
        ...
        stage('versys') {
            steps {
                ...
                sh """
                curl -X POST -F 'commitid=$commit_id' -F 'module=notificationservice' http://127.0.0.1:6200/new
                """
            }
        }
    }
    ...
    post ('Email notification') {
        success {
            echo "Image building success for notification service, sending email to $NOTIFY_EMAILS"
            notifyBuild('SUCCESS')
        }
        failure {
            echo "Image building failed for notification service, sending email to $NOTIFY_EMAILS"
            notifyBuild('FAILED')
        }
    }
}
