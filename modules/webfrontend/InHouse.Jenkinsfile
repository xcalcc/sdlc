pipeline {
    ...
    environment {
      git_url = "gitlab-address/web-frontend"
      branch = "$WEBFRONTEND_BRANCH_NAME"
      version = "$VERSION"
      registry = "$REGISTRY"
    ...
    stages {
        ...
        stage('clone repository') {
            steps {
              dir('modules/webfrontend/workdir') {
        ...
        stage('versys') {
            steps {
                ...
                sh """
                curl -X POST -F 'commitid=$commit_id' -F 'module=webpage' http://127.0.0.1:6200/new
                """
            }
        }
    }
    post ('Email notification') {
            success {
                echo "Image building success for web ui, sending email to $NOTIFY_EMAILS"
                notifyBuild('SUCCESS')
            }
            failure {
                echo "Image building failed for web ui, sending email to $NOTIFY_EMAILS"
                notifyBuild('FAILED')
            }
    }
}
